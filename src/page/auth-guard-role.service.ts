...
export class AuthGuardRole implements CanActivate{
	...
	// 	if((this.pageService.getUser().role == '0' || this.pageService.getUser().role == '9') && (url == '' || url == '/admin' || url == '/admin/home')){
	// 		this.router.navigate(['./admin/workbench/reception']);
	// 		return false;
	// 	}
	// 	// 那段角色，是超级管理员0还是普通角色
	// 	// 如果是超级管理员，获取所有权限
	// 	if(this.pageService.getUser().role == '0' || this.pageService.getUser().role == '9'){
	// 		return true;
	// 	}
	...
	// 	}else{
	// 		//获取userClinicRoles
	// 		var urlOptions = '?username=' + this.pageService.getUser().username
	// 			 + '&token=' + this.pageService.getUser().token
	// 			 + '&role_id=' + this.pageService.getUser().clinicRoleId;
	// 		this.pageService.authoritylist(urlOptions).then((data) => {
	// 			if(data.status == 'no'){
	// 				alert(data.errorMsg);
	// 			}else{
	...
	// 							var queryParams = this.pageService.getUrlParams(url);
	...
	// 					this.router.navigate(['.' + redirectUrl], {queryParams: this.pageService.getUrlParams(authority.url)});
	// 				}
	// 				return true;
	// 			}
	// 		}
